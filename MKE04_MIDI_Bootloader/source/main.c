///////////////////////////////////////////////////////////////////////////
//
// MKE04Z128xxx4 MIDI SYSEX BOOTLOADER
// Sixty-four pixels ltd / Jason Hotchkiss
//
// 1 	14oct19 port from MK02 bootloader
//
///////////////////////////////////////////////////////////////////////////
...
void (* const g_pfnVectors[])(void) = {
	...
	IntDefaultHandler,         // 45: Single interrupt vector for all sources
};
...
void fc_run() {
    __asm volatile ("cpsid i");
	while(!(FTMRE->FSTAT & 0x80)); 	// ensure flash module is idle
	FTMRE->FCLKDIV &= 0xe0;
	FTMRE->FCLKDIV |= 0x13; 		// divide flash clock for 20MHz bus clock
	FTMRE->FSTAT |= 0x30; 			// clear ACCERR and FPVIOL (write 1 to clear bits!)
	MCM->PLACR |= (1U<<16); 		// allow flash module to block if other access in progress
	FTMRE->FSTAT |= 0x80; 			// kick off the command
	while(!(FTMRE->FSTAT & 0x80)); 	// wait for CCIF to go high
